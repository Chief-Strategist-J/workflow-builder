...
import { Trash2, Settings, Brain, MessageSquare, Database } from 'lucide-react'
...
export const AgentNode: React.FC<AgentNodeProps> = ({
  ...
  const IconComponent = getIcon(node.type.icon)

  const getNodeColor = (iconName: string) => {
    switch (iconName) {
      case '🧠': return 'from-purple-500 to-pink-500'
      case '💬': return 'from-blue-500 to-cyan-500'
      case '🗄️': return 'from-green-500 to-emerald-500'
  ...
  return (
    ...
    >
      <div className={`
        relative bg-gradient-to-br ${getNodeColor(node.type.icon)} p-[2px] rounded-2xl
      ...
      `}>
        <div className="bg-gray-900/95 backdrop-blur-xl rounded-2xl p-4 w-[200px]">
          ...
          <div className="flex items-center gap-3 mb-2">
            <div className={`p-2 rounded-lg bg-gradient-to-br ${getNodeColor(node.type.icon)}`}>
              <IconComponent className="w-5 h-5 text-white" />
            </div>
            <div className="flex-1">
              <div className="text-white font-semibold text-sm">{node.type.name}</div>
              <div className="text-gray-400 text-xs">{node.type.description}</div>
            </div>
          </div>
      ...
      <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          ...
          className="p-1.5 bg-gradient-to-br from-red-500 to-pink-500 rounded-full shadow-lg hover:scale-110 transition-transform"
        >
          <Trash2 className="w-3 h-3 text-white" />
        </button>
        <button className="p-1.5 bg-gradient-to-br from-blue-500 to-purple-500 rounded-full shadow-lg hover:scale-110 transition-transform">
